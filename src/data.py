...
## for colorization
class Histology_colorization(Dataset):
    ...
    def __getitem__(self, index):
        img_path = os.path.join(self.image_dir, self.images[index])
        image = np.array(Image.open(img_path).convert('RGB'))
        image_grayscale =  np.array(Image.open(img_path).convert('L'), dtype=np.float32)
        image_grayscale[image_grayscale== 255.0] = 1.0

        if self.transform is not None:
            augmentations = self.transform(image=image, image_grayscale = image_grayscale)
            image = augmentations['image']
            image_grayscale = augmentations['image_grayscale']
