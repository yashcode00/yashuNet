...
from plot import vae_plot_reconstructed_images2
...
class Autoencoder:
    def __init__(self,train_dl:  DataLoader) -> None:
        ...
        self.feature_extractor = VGG19().to(self.gpu_id)
        ...
        for phase in ['train']:
            ...
            for images, _  in batch_iterator:
                ...
                if phase == "val":
                    ...
                    recon_img, mu, log_var = self.model.module.forward(images)

                    #VAE loss
                    kl_loss = kl_loss_func(mu, log_var)
                    mse_loss = self.loss_fn(recon_img, images)
                    
                    #Perception loss
                    feat_in = torch.cat((recon_img, images), 0)
                    feature_loss = self.feature_extractor(feat_in)
    ...
    def train(self):
        ...
        for epoch in range(self.n_epochs):
                ...
                if self.gpu_id == 0:
                    path = vae_plot_reconstructed_images2(self.model.module, self.train_dl, self.eval_path, f"{str(epoch)}.png")
                    try:
